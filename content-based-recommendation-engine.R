...
movie_recommendation <- function(){
  ...
  colnames(genres2) <- c("genre1", "genre2", "genre3")
  
  #get unique genres from all movies
  unique_genre1 <- NULL
  all_genre <- c(genres2$genre1,genres2$genre2,genres2$genre3)
  ...
  ratings$userId <- c(1)
  ratings <- ratings[,c("movieId", "selected_movie", "selected_rating", "userId")]
  ...
  result = matrix(0,nrow(unique_genre),1)
  ...
  recommended_movies_view <- recommended_movies_view[,c("Poster","Title","Info")]
